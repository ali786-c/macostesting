...
// src/authApi.ts
import { API_PATH } from "@easypark/shared";
...
function createAuthApi(http) {
  return {
    async login(payload) {
      try {
        const data = await http.post(API_PATH.AUTH_LOGIN, payload);
        return data;
      } catch (e) {
    ...
    async signup(payload) {
      try {
        const data = await http.post(API_PATH.USERS_REGISTER, payload);
        return data;
      } catch (e) {
        const err = normalizeError(e);
    ...
    async forgotPassword(email) {
      try {
        await http.post(
          `${API_PATH.USERS_FORGOT_PASSWORD}?email=${encodeURIComponent(email)}`,
          null
        );
      } catch (e) {
        const err = normalizeError(e);
    ...
    async resetPassword(payload) {
      try {
        await http.post(
          `${API_PATH.USERS_RESET_PASSWORD}?token=${encodeURIComponent(payload.token)}&newPassword=${encodeURIComponent(payload.newPassword)}`,
          null
        );
      } catch (e) {
        const err = normalizeError(e);
...
// src/placesApi.ts
import { API_PATH as API_PATH2 } from "@easypark/shared";
...
function createPlacesApi(http) {
  return {
    async getAll(params) {
      ...
      const data = await http.get(`${API_PATH2.PLACES}${query}`);
      return Array.isArray(data) ? data : data?.content ?? [];
    },
    async getById(placeId) {
      try {
        return await http.get(API_PATH2.PLACES_BY_ID(placeId));
      } catch (e) {
    ...
    async getOwnerCalendarOverview(userId) {
      try {
        const data = await http.get(
          `${API_PATH2.PLACES_OWNER_CALENDAR_OVERVIEW(userId)}?_t=${Date.now()}`
        );
...
import { API_PATH as API_PATH3 } from "@easypark/shared";
function createReservationsApi(http) {
  return {
    async getClientReservations(clientId) {
      try {
        const data = await http.get(
          API_PATH3.RESERVATIONS_CLIENT(clientId)
        );
        return Array.isArray(data) ? data : [];
      } catch (e) {
        throw normalizeError(e);
    ...
    async getOwnedReservations(userId) {
      try {
        const data = await http.get(
          API_PATH3.USERS_OWNED_RESERVATIONS(userId)
        );
        return Array.isArray(data) ? data : [];
      } catch (e) {
        throw normalizeError(e);
    ...
    async getById(reservationId) {
      try {
        return await http.get(
          API_PATH3.RESERVATIONS_BY_ID(reservationId)
        );
      } catch (e) {
        throw normalizeError(e);
      }
